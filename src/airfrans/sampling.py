...
def cell_sampling_2d(cell_points, cell_attr = None):
    ...
    Args:
        cell_points (torch.tensor): Vertices of the 2 dimensional cells. Shape (N, 4) for N cells with 4 vertices.
        cell_attr (torch.tensor, optional): Features of the vertices of the 2 dimensional cells. Shape (N, 4, k) for N cells with 4 edges and k features. 
    ...
    a0, a1 = torch.abs(torch.linalg.det(torch.hstack([v0[:, :2], v1[:, :2]]).reshape(-1, 2, 2))), \
                torch.abs(torch.linalg.det(torch.hstack([v2[:, :2], v3[:, :2]]).reshape(-1, 2, 2)))
    p = a0/(a0 + a1)
    index_triangle = torch.bernoulli(p)[:, None]
    u = torch.rand(size = (len(p), 2))
    ...
    if cell_attr is not None:
        t0, t1, t2 = torch.zeros_like(v0), index_triangle*v0 + (1 - index_triangle)*v2, index_triangle*v1 + (1 - index_triangle)*v3
    ...
    return torch.hstack([sampled_point[:, :2], sampled_attr]) if cell_attr is not None else sampled_point[:, :2]
...
def cell_sampling_1d(line_points, line_attr = None):
    ...
    Args:
        line_points (torch.tensor): Edges of the 1 dimensional cells. Shape (N, 2) for N cells with 2 edges.
        line_attr (torch.tensor, optional): Features of the edges of the 1 dimensional cells. Shape (N, 2, k) for N cells with 2 edges and k features.
            If given shape (N, 2) it will resize it automatically in a (N, 2, 1) tensor. Default: ``None``
    '''
    # Linear sampling
    u = torch.rand(size = (len(line_points), 1))
    ...
    return torch.hstack([sampled_point[:, :2], sampled_attr]) if line_attr is not None else sampled_point[:, :2]